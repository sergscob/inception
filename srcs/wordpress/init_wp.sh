...
if ! /usr/bin/php81 /usr/local/bin/wp core is-installed --allow-root --path="$WP_PATH"; then
    ...
    if ! /usr/bin/php81 /usr/local/bin/wp user get "${WP_SECOND_USER:-editor}" --allow-root --path="$WP_PATH" >/dev/null 2>&1; then
        echo "--Creating second user"
        /usr/bin/php81 /usr/local/bin/wp user create \
            "${WP_SECOND_USER:-editor}" "${WP_SECOND_USER_EMAIL:-editor@example.com}" \
            --user_pass="${WP_SECOND_USER_PASSWORD:-editor123}" \
            --role="${WP_SECOND_USER_ROLE:-editor}" \
            --allow-root \
            --path="$WP_PATH"
    ...
    sed -i "1a define('WP_REDIS_HOST', 'redis'); define('WP_REDIS_PORT', 6379);  define('WP_REDIS_DATABASE', 0); define('WP_CACHE', true);" "$WP_PATH/wp-config.php" 

    # echo "define('WP_REDIS_HOST', 'redis');" >> $WP_PATH/wp-config.php
    # echo "define('WP_REDIS_PORT', 6379);" >> $WP_PATH/wp-config.php
    # echo "define('WP_REDIS_CLIENT', 'phpredis');" >> $WP_PATH/wp-config.php
    # echo "define('WP_CACHE', true);" >> $WP_PATH/wp-config.php
    # echo "define('WP_REDIS_SCHEME', 'tcp');" >> $WP_PATH/wp-config.php
    ...
    # echo "  \$redis = new Redis();\$redis->connect('redis', 6379);var_dump(\$redis->ping());exit;" >> $WP_PATH/wp-config.php
    # sed -i "1a define('WP_REDIS_HOST', 'redis');" "$WP_PATH/wp-config.php"
...
cp $WP_PATH/wp-content/plugins/redis-cache/includes/object-cache.php $WP_PATH/wp-content/object-cache.php
